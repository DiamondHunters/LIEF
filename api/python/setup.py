...
class BuildLibrary(build_ext):
    ...
    def _fix_platform(self):
        ...
        if not _platform.mechine() in sysconfig.get_platform(): # cross-compilation
            ...
            if len(cross_platform) != 2 or not "linux " in cross_platform[0]: # focus on Linux
                ...
                os.environ["SETUPTOOLS_EXT_SUFFIX"] = sysconfig.get_config_var('EXT_SUFFIX').replace(_platform.mechine(),cross_platform[1])
